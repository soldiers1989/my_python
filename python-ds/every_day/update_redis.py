...
def insert_into_local(year,month,day):
    sql = 'select max(dau_time_strptime) from  hntx_mc_dau'
    cur.execute(sql)
    ret = cur.fetchone()
    last_update=ret['max(dau_time_strptime)']
    conn.commit()
    if last_update == None:
        last_update=1483200000
    else:
        last_update=ret['max(dau_time_strptime)']

    new_time = int(time.mktime(time.strptime("{}:{}:{}".format(year,month,day), "%Y:%m:%d")))
